...
let backgroundMusic = new Audio('background_music.mp3'); // Cargar la música de fondo
let playerY = 180;
let obstacleX = 400;
let obstacleY = 200;
let obstacleSpeed = 2; // Velocidad inicial de los obstáculos
let score = 0;
let trophies = 0;
let gameOver = false;
...
function main() {
    ...
    // Reproducir música de fondo solo cuando se haga clic en el lienzo
    canvas.addEventListener('click', playBackgroundMusic);
...
function playBackgroundMusic() {
    ...
    // Eliminar el oyente después de que se haya hecho clic para evitar que se vuelva a reproducir la música
    canvas.removeEventListener('click', playBackgroundMusic);
}
...
function update() {
    ...
    if (obstacleX < 0) {
        ...
        if (score % 10 === 0) {
            obstacleSpeed += 0.5; // Ajusta el valor según sea necesario
        }
    }
